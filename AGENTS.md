...
- 추천 Worker는 전체 Brief와 중요도·최신성으로 제한한 공통 Memory snapshot을 run당 한 번 읽어 단계들에 재사용한다. 역할별 Memory retrieval이나 별도 Behavior Context read/write/재작성 LLM을 production 경로에 다시 추가하지 않는다.
