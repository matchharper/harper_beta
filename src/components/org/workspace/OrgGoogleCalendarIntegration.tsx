...
export function OrgGoogleCalendarIntegration({
  ...
  return (
    <OrgSection aria-labelledby="personal-integrations-title">
      <OrgSectionHeader
        title={
          <span id="personal-integrations-title">Personal integrations</span>
        }
        description="내 Harper 계정에 연결하는 서비스예요. 다른 팀원과 공유되지 않아요."
      />
      <div className="flex flex-col gap-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-[13px] font-light leading-5 text-neutral-muted">
            지금은 계정 연결만 지원해요. 일정 조회나 생성, 초대 발송은 하지
            않아요.
          </p>
        </div>
      ...
      {!hasCallback && state === "disabled" ? (
        ...
        <p className="text-[13px] leading-5 text-neutral-muted">
          Google Calendar 연결이 만료되었거나 해제됐어요. 사용하려면 다시 연결해
          주세요.
        </p>
      ) : null}
      {callbackError ? (
        <OrgErrorState message={callbackError} className="mt-3" />
      ) : null}
      ...
      {mutationError ? (
        <OrgErrorState message={mutationError.message} className="mt-3" />
      ) : null}
