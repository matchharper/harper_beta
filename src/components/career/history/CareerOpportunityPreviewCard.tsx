...
export default function CareerOpportunityPreviewCard({
  item,
  onActivate,
  className,
  isRecommended = false,
  ...
  return (
    <ClickablePanel
      ...
      className={cn(
        "group relative flex min-w-0 shrink-0 cursor-pointer flex-col gap-4 rounded-[8px] border border-neutral-1000-a05 bg-bg-floating px-4 py-4 text-left transition-colors hover:border-neutral-400 hover:bg-bg-weak",
        className
      )}
      aria-label={t(
        "career.chat.opportunity_preview_cards.open_posting_label",
        "{companyName} {title} 포지션 열기",
    ...
    >
      {(isRecommended || isFit) && (
        <div className="absolute right-3 top-3 z-10 flex flex-wrap justify-end gap-1.5">
          {isRecommended && (
            <Badge>{t("career.company.jobs.recommended", "추천됨")}</Badge>
          )}
          {isFit && (
            <Badge tone="positive">
              {t("career.company.jobs.fit", "적합")}
            </Badge>
          )}
        </div>
      )}
