...
>(function ChatComposerCollapsedFrame(
  ...
  return (
    <MuteButton
      ref={ref}
      className={cn(
        "grid h-12 w-full gap-0.5 grid-cols-[auto_minmax(0,1fr)_auto] items-center rounded-[18px] border px-2 py-0 text-left transition-[background-color,border-color,box-shadow,opacity,transform] duration-200 ease-out active:scale-[0.99] motion-reduce:transition-none",
    ...
    >
      ...
      <span
        className={cn(
          "min-w-0 truncate pr-2 text-sm font-normal text-neutral-soft",
          textClassName
        )}
      >
        {children}
    ...
    >
      ...
      >
        ...
        <Textarea
          ...
          onFocus={handleFocus}
          className={cn(
            "min-h-[72px] min-w-0 resize-none border-none px-3.5 py-4 text-base leading-5 text-neutral-primary outline-none transition-[min-height,padding,opacity] duration-[240ms] ease-[cubic-bezier(0.22,1,0.36,1)] placeholder:text-neutral-placeholder disabled:cursor-not-allowed md:text-sm motion-reduce:transition-none",
            expanded
              ? "max-md:col-span-2 max-md:col-start-1 max-md:row-start-1 max-md:min-h-12 max-md:w-full max-md:px-3.5 max-md:py-3"
              : cn(
                  "flex items-center max-md:row-start-1 max-md:h-12 truncate",
                  mobileLeadingAction
