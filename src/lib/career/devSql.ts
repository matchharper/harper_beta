...
- talent_users: user_id uuid/text primary account id; email, name, headline, bio, location, current_location, resume_file_name, resume_storage_path, resume_links, resume_text, last_logined_at, created_at, updated_at.
...
- talent_contexts: id bigint, talent_id references talent_users.user_id, ref bigint, collection brief|memory, optional label/key, content text, revision, embedding metadata, timestamps, deleted_at. Brief and Memory are the active career-context source.
- talent_insights: legacy compatibility table retained during the cross-repository migration; new beta flows must not write it.
...
- talent_role_activity: id uuid, recommendation_id references talent_opportunity_recommendation.id, kind, content, metadata jsonb, created_at. Current Career writers append memo and saved_stage_changed activities; meetings and company request/response events remain in their source tables.
