...
‘{{name}}님, 안녕하세요. OptimizerAI에서 공동 창업자 겸 연구 엔지니어로 계셨고, 이전에는 띵스플로우에서 ML Scientist로 계시면서 인상적인 성과를 내셨습니다. 먼저 현재 **다음 커리어 스텝에 대해서 어떻게 생각하고 계신지** 말씀해주시겠어요?’
...
‘지금까지 말씀해주신 걸 보면, 특히 대규모 오디오 모델 연구와 실제 프로덕트 런칭까지의 경험이 {{name}}님 커리어의 큰 축이 된 것 같아요. 기술적 깊이도 있지만, 유저 인터뷰부터 기능 기획까지 직접 하셨던 점도 인상적이네요. 그렇다면 다음 회사에서는 연구 중심의 역할과 제품 중심의 역할 중 어느 쪽을 더 선호하시나요? 상관없다면 둘다 상관없다고 말해주세요.’

‘Co-founder로서 스타트업의 성장을 직접 이끌어 보셨고, 딥테크 인력들과 교류하며 산업단의 중요한 점을 배우셨다고 들었습니다. {{name}}님께서 **가장 중요하게 생각하는 팀 문화나 환경적인 요소 (예: 보상, 성장성, 기술 스택, 워라밸 등)** 가 있다면 무엇일까요?’
