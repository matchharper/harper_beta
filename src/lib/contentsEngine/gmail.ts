...
export async function sendGtmOutreachEmail(args: {
  ...
  const config = getGtmOutreachGmailConfig();
  if (args.from.trim().toLowerCase() !== config.fromEmail) {
    throw new Error(
      `Approved sender ${args.from} does not match configured Gmail sender ${config.fromEmail}`
    );
  }
...
export async function verifyGmailPubSubToken(authorization: string | null) {
  ...
  if (!payload?.email_verified || payload.email?.toLowerCase() !== expectedEmail) {
    throw new Error("Unexpected Pub/Sub caller");
  }
  return payload;
}
