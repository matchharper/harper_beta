...
const migration =
  ...
  readFileSync(
    "supabase/migrations/20260910120000_org_agent_progress_contact_notices.sql",
    "utf8"
  );
...
test("contact index is a read-only projection over existing communication sources", () => {
  ...
  assert.match(reader, /"list_company_contact_index_v2"/);
  assert.match(
    migration,
  ...
  assert.match(migration, /from public\.talent_progress progress/);
  assert.match(migration, /'internal_process_stopped_notified'/);
  assert.match(migration, /'company_request_followup_sent'/);
  assert.match(migration, /'Harper'::text as company_user_name/);
  assert.match(migration, /progress\.kind as workflow_status/);
...
test("deleted candidates are excluded from every contact list and detail source", () => {
  ...
  assert.equal((reader.match(/\.is\("deleted_at", null\)/g) ?? []).length, 3);
});

test("progress-backed notices remain readable from list contact references", () => {
  assert.match(reader, /const PROGRESS_NOTICE_KINDS = \[/);
  assert.match(reader, /"internal_process_stopped_notified"/);
  assert.match(reader, /"company_request_followup_sent"/);
  assert.match(reader, /async function readProgressNotices/);
  assert.match(reader, /sender: HARPER_ACTOR/);
});
