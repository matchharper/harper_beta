...
export type OrgMembershipAuthorityUpdateResponse = {
  authority: OrgMembershipRole;
  ok: true;
  userId: string;
};

export type OrgMemberProfileUpdateResponse = {
  name: string | null;
  ok: true;
  role: string;
  userId: string;
...
export type OrgAcceptedTalentItem = {
  acceptedAt: string;
  companyName: string;
...
export type OrgAcceptedTalentsResponse = {
  items: OrgAcceptedTalentItem[];
...
export type OrgCompanyTalentRequestFeedItem = {
  canCancel: boolean;
...
const EXTRA_INTERNAL_STAGE_TAGS = ["내부:보류", "내부:추천", "내부:거절"];
...
function hasOrgAllWorkspaceAccess(user: User) {
  return hasOrgWorkspaceAccessBypass(getUserEmail(user));
}

function isOrgAllWorkspaceAccessEmail(email: string | null | undefined) {
  return hasOrgWorkspaceAccessBypass(email);
}
...
async function fetchBoardProfileLabels(args: {
  ...
  const results = await Promise.all(
    chunkValues(uniqueTexts(args.talentIds)).map(async (talentIdChunk) => {
      ...
      return {
        educations: (educationResult.data ?? []) as TalentEducationRow[],
        experiences: (experienceResult.data ?? []) as TalentExperienceRow[],
      };
    })
  );

  for (const result of results) {
    for (const row of result.experiences) {
    ...
    for (const row of result.educations) {
...
function stageSortOrder(stage: OrgStageId, customStages: RoleStageRow[]) {
  ...
  if (stage === "archived") return 10_002;
  const stageId = getCustomStageDbId(stage);
...
function getVisibleOrgStage(args: {
...
}): { stage: OrgStageId; stageTag: string | null } | null {
  ...
  const implicitAcceptanceStage = getOrgImplicitAcceptanceStage({
    connectedByOrgAction: args.connectedByOrgAction,
    feedback: args.feedback,
    includeInternalAccepted: args.includeInternalAccepted,
    isInternalRecommendation: args.isInternalRecommendation,
    savedStage: args.savedStage,
  });
  return implicitAcceptanceStage
    ? { stage: implicitAcceptanceStage, stageTag: null }
    : null;
}
...
function buildMember(
  ...
  return {
    authority: normalizeOrgMembershipRole(membership.authority),
...
async function ensureOrgMembershipFromInvitation(args: {
  admin: SupabaseAdminClient;
  user: User;
  workspaceId: string;
}) {
  ...
  const membershipRole = normalizeOrgMembershipRole(invitation.role);
...
export async function assertOrgWorkspacePermission(args: {
  ...
  const authority = normalizeOrgMembershipRole(data.authority);
  const permissions = getOrgPermissions(authority);
  const allowed =
...
export async function leaveOrgWorkspace(args: {
  ...
  if (normalizeOrgMembershipRole(membership.authority) === "owner") {
...
export async function fetchOrgBootstrap(args: {
  ...
  if (requestedWorkspaceId && !hasAllWorkspaceAccess) {
    await ensureOrgMembershipFromInvitation({
      admin,
      user: args.user,
  ...
  const defaultWorkspaces = hasAllWorkspaceAccess
    ? await fetchDefaultOrgWorkspaces(admin)
  ...
  const requestedWorkspace =
    ...
    !defaultWorkspaces.some(
      (workspace) => workspace.company_workspace_id === requestedWorkspaceId
    )
      ? await fetchWorkspaceById(admin, requestedWorkspaceId)
      : null;
  const workspaces = requestedWorkspace
    ? [requestedWorkspace, ...defaultWorkspaces]
    : defaultWorkspaces;
...
export async function updateOrgMembershipAuthority(args: {
  authority: unknown;
  user: User;
  userId: string;
  workspaceId: string;
}): Promise<OrgMembershipAuthorityUpdateResponse> {
  const admin = getSupabaseAdmin();
  const userId = normalizeText(args.userId);
  const workspaceId = normalizeText(args.workspaceId);
  const rawAuthority = normalizeText(args.authority).toLowerCase();
  if (!userId || !workspaceId) {
    throw new OrgHttpError(400, "Missing required fields");
  }
  if (!["owner", "admin", "viewer"].includes(rawAuthority)) {
    throw new OrgHttpError(400, "올바른 권한을 선택해 주세요.");
  }
  const authority = rawAuthority as OrgMembershipRole;
  await assertOrgWorkspacePermission({
    admin,
    permission: "manage_members",
    user: args.user,
  ...
  const previousAuthority = normalizeOrgMembershipRole(membership.authority);
  if (previousAuthority === "owner" && authority !== "owner") {
  ...
  const { error } = await (admin.from("company_user_workspace" as any) as any)
    ...
    .eq("id", membership.id);
  if (error) throw error;
  return { authority, ok: true, userId };
}

function normalizeOrgMemberProfileField(
  value: unknown,
  label: string,
  maxLength: number
) {
  ...
  if (!normalized) {
    throw new OrgHttpError(400, `${label}을(를) 입력해 주세요.`);
  }
...
export async function updateOrgMemberProfile(args: {
  ...
  userId?: string | null;
  workspaceId: string;
}): Promise<OrgMemberProfileUpdateResponse> {
  const admin = getSupabaseAdmin();
  const workspaceId = normalizeText(args.workspaceId);
  const requestedUserId = normalizeText(args.userId);
  const userId = requestedUserId || args.user.id;
  const role = normalizeOrgMemberProfileField(args.role, "직함", 160);
  ...
  await assertOrgWorkspacePermission({
    admin,
    permission: requestedUserId ? "manage_members" : "view",
    user: args.user,
    workspaceId,
  });
  ...
  if (!requestedUserId) {
    ...
    if (!updatedUser) {
      throw new OrgHttpError(404, "사용자 프로필을 찾지 못했습니다.");
    }
  } else {
    ...
    name = normalizeNullableText(targetUser?.name);
  }
  ...
  return { name, ok: true, role, userId };
}
...
async function fetchOrgConnectedRecommendationIdsForPage(args: {
  admin: SupabaseAdminClient;
...
async function assertOrgTalentVisibleInWorkspace(args: {
  ...
  const roleById = new Map(roleRows.map((role) => [role.role_id, role]));
    ...
    Boolean(
      getVisibleOrgStage({
        ...
        isInternalRecommendation:
          normalizeText(
            roleById.get(recommendation.role_id)?.source_type
...
export async function fetchOrgAcceptedTalents(args: {
  ...
  type AcceptedRecommendationRow = Pick<
    RecommendationRow,
    "feedback" | "feedback_at" | "id" | "role_id" | "saved_stage" | "talent_id"
  > & {
    role: AcceptedRoleRow | AcceptedRoleRow[] | null;
  };
  const recommendationRows = (data ?? []) as AcceptedRecommendationRow[];
  const recommendationIds = recommendationRows.map((row) => row.id);
  const talentIds = uniqueTexts(recommendationRows.map((row) => row.talent_id));
  const roleById = new Map<string, AcceptedRoleRow>();
  ...
  const items = recommendationRows.flatMap((row): OrgAcceptedTalentItem[] => {
    const talent = talentById.get(row.talent_id);
    const acceptedAt = row.feedback_at;
    ...
    const workspace = role
      ? Array.isArray(role.workspace)
        ...
        : role.workspace
      : null;
    if (!talent || !acceptedAt || !role || !workspace) return [];
  ...
  const totalCount = count ?? offset + recommendationRows.length;
  const nextOffset =
    offset + ORG_ACCEPTED_TALENTS_PAGE_SIZE < totalCount
      ? offset + ORG_ACCEPTED_TALENTS_PAGE_SIZE
      : null;
...
export async function fetchOrgBoardProfileLabels(args: {
  recommendationIds: string[];
  user: User;
  workspaceId: string;
...
async function sendOrgIntroEmail(args: {
  ...
  if (cc.length === 0) {
    throw new OrgHttpError(
      400,
      "소개 이메일에는 후보자 이메일과 다른 회사 담당자 이메일이 필요합니다."
    );
  }
...
function getStageTagForInsert(stage: OrgStageId) {
  ...
  if (stage === "archived") return STAGE_TAG_BY_STAGE.archived;
...
export async function updateOrgConnectionConfirmationEmail(args: {
  ...
  const roleId = normalizeText(args.roleId);
  const talentId = normalizeText(args.talentId);
  const queueId = normalizeText(args.queueId);
  if (!workspaceId || !roleId || !talentId || !queueId) {
    throw new OrgHttpError(400, "Missing required fields");
  }
  ...
  const roleRows = await fetchRoleRowsForWorkspace(admin, workspaceId);
  if (!roleRows.some((role) => role.role_id === roleId)) {
    throw new OrgHttpError(404, "Role not found");
  }
  const emails = await fetchInternalConnectionConfirmationEmails({
    admin,
    roleId,
    talentId,
  });
  if (!emails.some((item) => item.id === queueId)) {
    throw new OrgHttpError(404, "연결 확정 안내 메일을 찾을 수 없습니다.");
  }

  try {
    return await updateInternalConnectionConfirmationEmail({
      action: args.action,
      actorEmail: getUserEmail(args.user),
      admin,
      queueId,
    ...
    if (error instanceof ConnectionConfirmationEmailError) {
      throw new OrgHttpError(error.status, error.message);
    }
    throw error;
  }
}

export type OrgCompanyTalentRequestCancelResponse = {
  cancelledAt: string | null;
  ok: true;
  requestId: string;
  status: "cancelled";
...
export async function cancelOrgCompanyTalentRequest(args: {
  requestId: string;
  roleId: string;
  talentId: string;
  user: User;
  workspaceId: string;
}): Promise<OrgCompanyTalentRequestCancelResponse> {
  const admin = getSupabaseAdmin();
  const workspaceId = normalizeText(args.workspaceId);
  const roleId = normalizeText(args.roleId);
  const talentId = normalizeText(args.talentId);
  const requestId = normalizeText(args.requestId);
  if (!workspaceId || !roleId || !talentId || !requestId) {
    throw new OrgHttpError(400, "Missing required fields");
  }
  ...
  try {
    const result = await cancelCompanyTalentRequest({
      admin: admin as any,
      requestId,
      roleId,
      talentId,
    ...
    const message =
      error && typeof error === "object" && "message" in error
        ...
        : String(error);
    if (message.includes("company_talent_request_not_found")) {
      throw new OrgHttpError(404, "후보자 문의 요청을 찾을 수 없습니다.");
    }
    if (message.includes("company_talent_request_not_cancellable")) {
      throw new OrgHttpError(
        409,
        "이미 발송이 시작됐거나 완료된 문의는 취소할 수 없습니다."
      );
    }
    throw error;
  }
...
export async function fetchOrgTalentDetail(args: {
  ...
  const [detailWorkspace, members, roleRows] = await Promise.all([
    ...
    fetchRoleRowsForWorkspace(admin, workspaceId),
  ]);
  if (!detailWorkspace) throw new OrgHttpError(404, "Workspace not found");
  ...
  const [customStages, tagsByKey, connectedRecommendationIds] =
    await Promise.all([
      ...
      fetchOrgConnectedRecommendationIds({
        admin,
        roleIds: [recommendation.role_id],
      }),
    ]);
  ...
  return {
    ...
    workspace: {
      companyName: detailWorkspace.company_name,
      workspaceId: detailWorkspace.company_workspace_id,
    },
  };
}
