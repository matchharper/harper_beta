...
export const COMPANY_SERVICE_FAQ_ITEMS = [
  ...
  },
  {
    ...
    answer:
      "네. 연결을 수락하거나 거절하기 전에 필요한 경력 내용을 묻거나 최신 이력서를 요청할 수 있어요. Harper가 대신 여쭤볼 내용을 자연스러운 연락 문구로 먼저 보여드리고, 회사가 본문을 확인한 뒤에만 후보자에게 보내요. 후보자는 답하지 않거나 요청을 거절할 수도 있어요. 답변이나 자료 제출이 없고 요청이 계속 유효하면, 최초 전달 후 최소 3일이 지난 뒤 Harper가 부담되지 않게 follow-up 이메일을 한 번 보내요. 답변이 오면 요청을 시작한 대화에서 알려드리므로, 그 내용을 확인한 뒤 연결 여부를 결정하면 돼요.",
  ...
  },
  {
    ...
    answer:
      "잠시 쉬었다가 채용을 다시 이어갈 계획이라면 중단을 선택해 주세요. 새 추천만 멈추고, 이미 진행 중인 후보자는 그대로 검토할 수 있어요. 종료를 선택하면 새 추천이 멈추고 역할이 종료 상태로 바뀌어요. 이 상태 변경과 동시에 안내가 발송되는 것은 아니지만, 유예기간이 지나면 수락 후 진행 중인 후보자에게 Harper가 종료를 안내하고 Position을 닫아요. 1차 인터뷰 같은 중간 단계도 동일하게 처리하며, 최종 오퍼 단계만 자동 종료에서 제외해 따로 관리해요.",
  ...
  },
  {
    ...
    answer:
      "네. 일정 담당자가 Google Calendar를 연결하고 인터뷰 가능한 시간을 설정해 두면 Harper가 일정 조율을 맡을 수 있어요. 연결된 Calendar에서 이미 바쁜 시간을 제외한 선택지를 후보자에게 보내고, 후보자가 가능한 시간을 고르면 그중 한 시간을 확정해 후보자와 회사 참석자에게 Calendar 초대와 Google Meet 링크를 전달해요. Google Calendar 연결이나 담당자의 가능 시간 설정이 없다면 먼저 준비해야 하며, 후보자를 Pipeline의 다른 단계로 옮기는 것만으로 일정 요청이 자동 발송되지는 않아요.",
