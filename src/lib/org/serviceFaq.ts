...
export const COMPANY_SERVICE_FAQ_ITEMS: readonly CompanyServiceFaqItem[] = [
  {
    ...
    answer:
      "Harper는 단순히 후보자 목록을 보여주는 서비스가 아니라, 회사와 함께 채용을 진행하는 파트너예요. 채용할 역할과 판단 기준을 정리하고, 그 기준에 맞는 분들의 경력을 검토한 뒤 후보자에게 회사와 역할을 설명하고 대화 의향을 확인해요. 대화해 보고 싶다고 답한 후보자를 Inbox의 연결 대기 상태로 회사에 소개해요. 회사가 연결을 수락한 뒤에는 Pipeline에서 다음 전형과 결과를 관리할 수 있어요. 별도의 먼저 제안 가능한 후보 탭에서는 아직 이 역할을 추천받지 않은 후보에게 회사가 먼저 제안할지 결정할 수 있어요.",
  ...
  },
  {
    ...
    question: "연결 대기와 먼저 제안 가능한 후보는 어떻게 다른가요?",
    answer:
      "연결 대기는 후보자가 회사와 역할에 대한 설명을 듣고 대화해 볼 의사를 밝힌 뒤, 회사의 결정을 기다리는 상태예요. 단순히 검색에서 찾은 사람이나 Harper가 일방적으로 올린 후보자가 아니에요. 추천 이유와 자료를 검토한 뒤 Inbox에서 연결을 수락할지 거절할지 알려 주세요. 결정하기에 정보가 부족하다면 먼저 Harper에게 후보자 확인을 요청할 수 있어요. 먼저 제안 가능한 후보는 아직 이 역할을 추천받지 않은 사람에게 회사가 먼저 연락해 보도록 제안하는 별도 흐름이에요. 먼저 제안하기를 선택하면 Harper가 회사의 관심 이유를 담아 연락하고, 후보자가 수락하면 미리 정한 담당자와 소개 이메일로 연결해 첫 단계로 이동해요. 제안을 요청한 뒤에도 같은 탭에 남으며, 제안 준비 중인지 후보자 답변 대기인지 카드에 표시돼요. 제안하지 않기를 선택하면 후보자에게 연락하지 않고 목록에서 제외해요.",
  ...
  },
  {
    ...
    answer:
      "네. 연결 대기에서 연결 여부를 정하기 전에도, 연결 후 채용 절차를 진행하는 동안에도 필요한 경력 내용을 묻거나 최신 이력서를 요청할 수 있어요. 다만 먼저 제안 가능한 후보는 아직 후보자의 수락 전이므로, 먼저 역할을 제안하고 수락을 기다려야 해요. Harper가 대신 여쭤볼 내용을 자연스러운 연락 문구로 먼저 보여드리고, 회사가 본문을 확인한 뒤에만 후보자에게 보내요. 후보자는 답하지 않거나 요청을 거절할 수도 있어요. 답변이나 자료 제출이 없고 요청이 계속 유효하면, 최초 전달 후 최소 3일이 지난 뒤 Harper가 부담되지 않게 후속 확인 이메일을 한 번 보내요. 답변이 오면 요청을 시작한 대화에서 알려드리므로, 새로 확인한 내용을 바탕으로 연결 여부나 다음 진행을 결정하면 돼요.",
