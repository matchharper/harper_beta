...
function getConfiguredCronSecrets() {
  return [
    process.env.WEEKLY_USER_STATS_CRON_SECRET?.trim(),
...
async function handleWeeklyUserStats(req: NextRequest) {
  ...
  if (configuredSecrets.length === 0) {
    return NextResponse.json(
      {
        error:
          "Missing WEEKLY_USER_STATS_CRON_SECRET, DAILY_USER_STATS_CRON_SECRET, or CRON_SECRET",
      },
      { status: 500 }
    );
  }
  ...
  const weekStartDate = resolveWeeklyUserStatsStartDate(
...
export async function GET(req: NextRequest) {
  try {
    return await handleWeeklyUserStats(req);
  } catch (error) {
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Failed to send weekly user stats",
      },
      { status: 500 }
    );
  }
