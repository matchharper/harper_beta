  ...
  return (
    <div
      ...
      className={`rounded-sm flex flex-row gap-2 items-start justify-between px-2 py-2 mb-2 group transition-all duration-200 ${
        isInteractive ? "cursor-pointer hover:bg-bg-weak" : ""
      }`}
    >
      <div className="flex flex-col items-start justify-start w-full">
        <div className="w-full text-sm flex flex-row items-start justify-between gap-2 font-light">
          <div className="flex-1 min-w-0 text-base wrap-break-word">
            {title}
          </div>
          <div className="shrink-0 whitespace-nowrap text-neutral-muted text-sm pt-0.5">
            {citation_num >= 0 ? `${citation_num} Cite` : ""}
          </div>
        </div>
        <div className="w-full flex flex-row items-center justify-between gap-2">
          <div className="text-sm text-neutral-muted mt-1 flex flex-row items-center justify-start">
            {mapped && (
              <div className="font-medium text-left text-neutral-primary mr-2">
                {mapped}
              </div>
            )}
            <div>{venue}</div>
          </div>
          <div className="shrink-0 whitespace-nowrap text-neutral-muted text-[15px] pt-0.5">
            {year}
          </div>
        </div>
      </div>
      {isInteractive ? (
        <div className="inline-flex items-center gap-1 text-sm text-neutral-muted hover:underline">
          <ArrowUpRight
