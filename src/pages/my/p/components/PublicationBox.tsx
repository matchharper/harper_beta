  ...
  return (
    <div
      ...
      className={`rounded-sm flex flex-row gap-2 items-start justify-between px-2 py-2 mb-2 group transition-all duration-200 ${
        isInteractive ? "cursor-pointer hover:bg-beige500/55" : ""
      }`}
    >
      <div className="flex flex-col items-start justify-start w-full">
        ...
        <div className="w-full flex flex-row items-center justify-between gap-2">
          <div className="text-sm text-beige900/55 mt-1 flex flex-row items-center justify-start">
            {mapped && (
              <div className="font-medium text-left text-beige900 mr-2">
                {mapped}
              </div>
            )}
            <div>{venue}</div>
          </div>
          <div className="shrink-0 whitespace-nowrap text-beige900/55 text-[15px] pt-0.5">
            {year}
          </div>
        </div>
      </div>
