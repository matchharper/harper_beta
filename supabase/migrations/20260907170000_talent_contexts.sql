...
create table if not exists public.talent_contexts (
  ...
  constraint talent_contexts_importance_check
    check (
...
create index if not exists talent_contexts_active_memory_priority_idx
...
-- Unified Memory now owns durable talent context. Stop producing the legacy
-- Behavior Context change queue after its existing text has been copied above.
-- Keep the old tables temporarily for rollback/audit compatibility; no active
-- application or worker reader depends on them after this rollout.
