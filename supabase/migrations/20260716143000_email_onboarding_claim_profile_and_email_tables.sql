...
begin
  ...
  if source_user_id = target_user_id then
    ...
    update public.talent_users
       set email = coalesce(nullif(trim(coalesce(email, '')), ''), normalized_target_email, source_row.email),
           name = coalesce(nullif(trim(coalesce(name, '')), ''), source_row.name, normalized_target_name),
           ...
           current_location = coalesce(nullif(trim(coalesce(current_location, '')), ''), source_row.current_location),
           location = coalesce(nullif(trim(coalesce(location, '')), ''), source_row.location, source_row.current_location),
    ...
    values (
      ...
      coalesce(source_row.name, normalized_target_name),
      coalesce(normalized_target_profile_picture, source_row.profile_picture),
      source_row.headline,
      source_row.bio,
      source_row.current_location,
      coalesce(source_row.location, source_row.current_location),
  ...
  if source_user_id <> target_user_id then
    ...
    update public.career_email_messages
    ...
    delete from public.talent_opportunity_fit source_fit
     using public.talent_opportunity_fit target_fit
     where source_fit.talent_id = source_user_id
       and target_fit.talent_id = target_user_id
       and target_fit.opportunity_id = source_fit.opportunity_id;
    ...
    delete from public.talent_opportunity_tag source_tag
     using public.talent_opportunity_tag target_tag
     where source_tag.talent_id = source_user_id
       and target_tag.talent_id = target_user_id
       and target_tag.opportunity_id is not distinct from source_tag.opportunity_id
       and lower(btrim(target_tag.tag)) = lower(btrim(source_tag.tag));
    ...
    if exists (select 1 from public.talent_setting where user_id = target_user_id) then
      update public.talent_setting target_setting
         set profile_visibility = case
             ...
             preferred_locale = coalesce(source_setting.preferred_locale, target_setting.preferred_locale),
             updated_at = now_ts
