create or replace function public.get_ops_talent_metrics_v1(
  p_from date,
  p_to date,
...
begin
  ...
  included_users as materialized (
    select
      u.user_id,
      u.created_at
    from public.talent_users u
    where u.deleted_at is null
      and split_part(lower(coalesce(u.email, '')), '@', 2) <> 'matchharper.com'
      and not exists (
        select 1
        from unnest(coalesce(p_excluded_email_terms, '{}'::text[])) term(value)
        where nullif(btrim(term.value), '') is not null
          and position(lower(btrim(term.value)) in lower(coalesce(u.email, ''))) > 0
      )
  ),
  ...
  activity_events as materialized (
    select message.user_id, message.created_at as occurred_at, true as is_interaction
    from public.talent_messages message
    join included_users users on users.user_id = message.user_id
    where message.role = 'user'
    ...
    select log.user_id, log.created_at, left(coalesce(log.type, ''), 7) = 'career_'
    ...
    select recommendation.talent_id, recommendation.viewed_at, true
    ...
    where recommendation.viewed_at >= v_activity_start
      and recommendation.viewed_at < v_effective_end
      and not exists (
        select 1 from test_only_roles test_role where test_role.role_id = recommendation.role_id
      )

    union all

    select recommendation.talent_id, recommendation.clicked_at, true
    from public.talent_opportunity_recommendation recommendation
    join included_users users on users.user_id = recommendation.talent_id
    where recommendation.clicked_at >= v_activity_start
      and recommendation.clicked_at < v_effective_end
      and not exists (
        select 1 from test_only_roles test_role where test_role.role_id = recommendation.role_id
      )
    ...
    select recommendation.talent_id, recommendation.feedback_at, true
    from public.talent_opportunity_recommendation recommendation
    join included_users users on users.user_id = recommendation.talent_id
    where recommendation.feedback_at >= v_activity_start
      and recommendation.feedback_at < v_effective_end
      and not exists (
        select 1 from test_only_roles test_role where test_role.role_id = recommendation.role_id
      )
  ),
  ...
  period_metrics as (
    select
      ...
      (
        ...
        where message.occurred_at >= period.start_at and message.occurred_at < period.end_at
      ) as message_count,
      (
      ...
      (
        ...
        where activity.is_interaction
          and activity.occurred_at >= period.start_at
          and activity.occurred_at < period.end_at
      ) as interaction_talent_count,
      (
        select count(distinct activity.user_id)::bigint
        from activity_events activity
        where activity.occurred_at >= greatest(period.start_at, period.end_at - interval '1 day')
          and activity.occurred_at < period.end_at
      ) as dau,
      (
        select count(distinct activity.user_id)::bigint
        from activity_events activity
        where activity.occurred_at >= greatest(period.start_at, period.end_at - interval '7 days')
          and activity.occurred_at < period.end_at
      ) as wau,
      (
        select count(distinct activity.user_id)::bigint
        from activity_events activity
        where activity.occurred_at >= period.end_at - interval '14 days'
          and activity.occurred_at < period.end_at - interval '7 days'
      ) as retention_base_talent_count,
      (
        select count(*)::bigint
        from (
          select distinct previous.user_id
          from activity_events previous
          where previous.occurred_at >= period.end_at - interval '14 days'
            and previous.occurred_at < period.end_at - interval '7 days'
            and exists (
              select 1
              from activity_events current_activity
              where current_activity.user_id = previous.user_id
                and current_activity.occurred_at >= greatest(
                  period.start_at,
                  period.end_at - interval '7 days'
                )
                and current_activity.occurred_at < period.end_at
            )
        ) retained
      ...
      (
        ...
        where stage.pending_at >= period.start_at and stage.pending_at < period.end_at
      ) as pending_connection_talent_count,
      (
        select count(distinct stage.talent_id)::bigint
        from pair_stage_first stage
        where stage.connected_at >= period.start_at and stage.connected_at < period.end_at
  ...
  period_json as (
    select
      metric.period_key,
      jsonb_build_object(
        'averageSentCharactersPerSender', metric.message_character_count / nullif(metric.message_sender_count, 0),
        'averageSentMessagesPerSender', metric.message_count::numeric / nullif(metric.message_sender_count, 0),
  ...
  message_buckets as (
    select
      ...
      count(distinct message.user_id)::bigint as sender_count,
      coalesce(sum(message.character_count), 0)::numeric as character_count
    from scoped_messages message
    where message.occurred_at >= v_start and message.occurred_at < v_effective_end
    group by 1
  ),
  interaction_buckets as (
    select
      date_trunc(p_interval, activity.occurred_at at time zone 'Asia/Seoul')::date as bucket_start,
      count(distinct activity.user_id)::bigint as talent_count
    from activity_events activity
    where activity.is_interaction
      and activity.occurred_at >= v_start
      and activity.occurred_at < v_effective_end
    group by 1
  ),
  decision_buckets as (
    select
    ...
    from latest_decisions decision
    where decision.decided_at >= v_start and decision.decided_at < v_effective_end
    group by 1
  ),
  internal_buckets as (
    select
  ...
  trend_json as (
    select coalesce(
      jsonb_agg(
        jsonb_build_object(
          'averageSentCharactersPerSender', message.character_count / nullif(message.sender_count, 0),
          'averageSentMessagesPerSender', message.message_count::numeric / nullif(message.sender_count, 0),
  ...
  daily_activity_counts as (
    select
      date_trunc('day', activity.occurred_at at time zone 'Asia/Seoul')::date as activity_date,
      count(distinct activity.user_id)::bigint as talent_count
    from activity_events activity
    where activity.occurred_at >= v_start
      and activity.occurred_at < v_effective_end
    group by 1
  ),
  ...
  weekly_activity as materialized (
    select distinct
      date_trunc('week', activity.occurred_at at time zone 'Asia/Seoul')::date as cohort_start,
      activity.user_id
    from activity_events activity
  ),
  weekly_counts as (
...
revoke all on function public.get_ops_talent_metrics_v1(date, date, text, text[])
  from public;
grant execute on function public.get_ops_talent_metrics_v1(date, date, text, text[])
  to service_role;
