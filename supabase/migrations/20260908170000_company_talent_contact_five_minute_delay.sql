...
-- Keep a short operational buffer for cancellation and queue pickup, but do
-- not make company users wait twenty minutes after they approve exact copy.
...
comment on function public.enqueue_company_talent_request_v1(
  uuid, uuid, uuid, uuid, bigint, boolean, text
) is 'Queues standard candidate contact five minutes after creation.';

comment on function public.schedule_company_talent_request_v1(
  uuid, uuid, uuid, uuid, integer, text
) is 'Queues approved candidate-contact copy after five minutes, or immediately when explicitly authorized.';
