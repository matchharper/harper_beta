...
create or replace view public.behavior_context_experiment_metrics as
with run_delivery as (
  select distinct delivery.discovery_run_id
  ...
  select
    run.id,
    run.context_variant,
    coalesce(run.context_metrics->>'assignedVariant', run.context_variant) as assigned_variant,
    run.status,
    run.elapsed_seconds,
    nullif(run.coverage->>'llmEstimatedCostUsd', '')::numeric as estimated_cost_lower_bound_usd,
    case
      when coalesce((run.coverage#>>'{llmUsage,usageComplete}')::boolean, false)
        then nullif(run.coverage->>'llmEstimatedCostUsd', '')::numeric
      else null
    end as estimated_cost_usd,
    case
      when coalesce((run.coverage#>>'{llmUsage,usageComplete}')::boolean, false)
        then nullif(run.coverage#>>'{llmUsage,costSummary,inputTokens}', '')::numeric
      else null
    end as input_tokens,
    case
      when coalesce((run.coverage#>>'{llmUsage,usageComplete}')::boolean, false)
        then nullif(run.coverage#>>'{llmUsage,costSummary,outputTokens}', '')::numeric
      else null
    end as output_tokens,
    coalesce((run.coverage#>>'{llmUsage,usageComplete}')::boolean, false) as usage_complete,
    (delivery.discovery_run_id is not null) as sent
  from public.opportunity_discovery_run run
  left join run_delivery delivery on delivery.discovery_run_id = run.id
  where run.context_metrics ? 'requestedVariant'
), recommendation_rollup as (
  select
    rec.discovery_run_id,
    count(*) as recommendation_count,
    count(*) filter (where rec.feedback is not null or rec.feedback_reason is not null) as feedback_count,
    count(*) filter (where lower(coalesce(rec.feedback, '')) in ('like', 'liked', 'positive')) as positive_count,
    count(*) filter (where lower(coalesce(rec.feedback, '')) in ('dislike', 'disliked', 'negative')) as negative_count
  from public.talent_opportunity_recommendation rec
  group by rec.discovery_run_id
)
select
  run.context_variant,
  run.assigned_variant,
  count(*) as run_count,
  count(*) filter (where run.assigned_variant <> run.context_variant) as fallback_run_count,
  count(*) filter (where run.status in ('completed', 'partial')) as completed_run_count,
  count(*) filter (where run.status = 'failed') as failed_run_count,
  count(*) filter (where run.sent) as sent_run_count,
  coalesce(sum(rec.recommendation_count) filter (where run.sent), 0)::bigint as recommendation_count,
  coalesce(sum(rec.feedback_count) filter (where run.sent), 0)::bigint as feedback_count,
  coalesce(sum(rec.positive_count) filter (where run.sent), 0)::bigint as positive_count,
  coalesce(sum(rec.negative_count) filter (where run.sent), 0)::bigint as negative_count,
  round(
    coalesce(sum(rec.feedback_count) filter (where run.sent), 0)::numeric /
    nullif(coalesce(sum(rec.recommendation_count) filter (where run.sent), 0), 0),
    4
  ) as feedback_rate,
  round(
    coalesce(sum(rec.positive_count) filter (where run.sent), 0)::numeric /
    nullif(coalesce(sum(rec.feedback_count) filter (where run.sent), 0), 0),
    4
  ) as positive_feedback_rate,
  round(avg(run.elapsed_seconds), 3) as average_elapsed_seconds,
  count(run.elapsed_seconds) as elapsed_measured_run_count,
  round(avg(run.estimated_cost_usd), 6) as average_estimated_cost_usd,
  round(avg(run.estimated_cost_lower_bound_usd), 6) as average_recorded_cost_lower_bound_usd,
  count(run.estimated_cost_usd) as cost_measured_run_count,
  count(*) filter (where run.usage_complete) as usage_complete_run_count,
  round(avg(run.input_tokens), 1) as average_input_tokens,
  round(avg(run.output_tokens), 1) as average_output_tokens
from run_rollup run
left join recommendation_rollup rec on rec.discovery_run_id = run.id
group by run.context_variant, run.assigned_variant;

grant select on public.behavior_context_experiment_metrics to service_role;
