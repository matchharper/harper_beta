...
2. 표를 보내는 workspace 안에서는 후보자 소개 대상이 없거나 연결 결정 대기가 0명인 Role도 포함한다. 단, 해당 workspace 전체의 연결 결정 대기가 0명이고 72시간 안에 시작하는 확정 미팅도 없으면 표와 리마인드를 보내지 않는다.
...
6. 새 후보자 소개가 있으면 같은 top-level 메시지에 표를 붙인다. 소개가 없어도 workspace 전체에 연결 결정 대기가 있거나 72시간 안에 시작하는 확정 미팅이 있으면 표와 리마인드를 보낸다.
