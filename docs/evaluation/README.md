...
| [company-side-conversational-qa](company-side-conversational-qa/README.md) | 실제 Slack 회사 사용자 대화에서 조회·후보자 연락·답장 확인·일정·Role 이동·수정·일상 대화의 전체 UX | 최신 회귀 세트 `cases-v4.json` 5개 scenario·회사 발화 31개; 실제 계정 매핑과 원문은 local-only | `z-test-harper` Slack plugin + 로컬 company-side LLM worker + 허용된 테스트 Gmail/일정 E2E | 2026-09-15 실사용 완료, NO-GO; 실행 중 source drift 발생 |
...
| [career-coaching-dialogue](career-coaching-dialogue/README.md) | Career 음성 통화의 음성 입출력을 제외한 코칭 대화 내용과 마무리 품질 | 비식별 synthetic challenge `cases-v1.json` 4개와 human-review gold v1 | `scripts/evalCareerCoachingDialogue.ts` | 2026-09-17 gpt-5.6-terra high v4: 4/4 content-only gate 통과, critical failure 0 |
| [career-company-research](career-company-research/README.md) | 회사 조사·합류 판단·경로 의존성·역량 전이 가능성·다음 선택지·지연 | 회사명만 캡처한 기존 5개와 합성 challenge 2개 `cases-v2.json`, human-review `gold-v4.json` | `scripts/evalCareerCompanyResearch.ts` | schema v7 공개 조사 Luna low·개인화 Terra high; 새 2건 반복 진단과 문서 본문 UI 검증, 전체 7건 gate는 미실행 |
