...
Do not invent missing facts or infer ability, compensation, company preference, or company interest from prestige, a title, a name, nationality, or another demographic proxy. Do not use protected traits. A missing fact is uncertainty; an explicit contradiction is evidence.

Apply this practical location convention consistently: when the talent is based in the role's target country or location and also has education or employment history in that same country or location, treat ordinary local-language ability, location feasibility, and ordinary work eligibility as satisfied. Do not require a particular visa category, permanent residence, or transferability unless the role explicitly requires that exact legal status. Explicit contrary evidence and special legal requirements such as citizenship, clearance, or a professional license still override this convention. Do not infer any additional language that is not the ordinary local language of that location.
...
Judge capabilities by the underlying work, not by literal keyword identity. A demonstrated adjacent language, framework, tool, or platform may satisfy an exact-name requirement when the evidence shows equivalent end-to-end responsibility and the skill is reasonably transferable. Do not create a hold solely because an otherwise strong candidate used an adjacent implementation technology. Preserve a hard mismatch only when the role makes the exact credential or technology genuinely non-substitutable, or when the underlying work itself is missing.
...
Question: Based on the talent's durable preferences and behavior, is this opportunity likely to be acceptable and satisfying to the talent?

Assume every supplied talent is actively exploring a new job now. Ignore whether the talent is currently employed, how recently they joined a company, notice period, stated search intensity, temporary availability, timing, or an off-market/pause statement. Those facts must not lower B and must not be used to suppress an otherwise relevant opportunity in this evaluation.

Consider explicit role/function and seniority preferences, desired scope and ownership, company stage or quality preferences, compensation, location and work mode, employment type, plus likes, rejections, accepted introductions, and other role/company preference behavior. Career history is only cautious supporting evidence for preference; ability to do the work is not itself evidence that the talent wants it.
...
- `unfit`: an explicit durable preference, hard constraint, or role/company-specific action clearly conflicts with the opportunity even under the assumption that the talent is actively job-seeking.
