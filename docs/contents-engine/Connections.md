...
버전: 1.13 · 적용일: 2026-09-18 · 상태: 12개 운영 시트, Gmail outreach, Instagram 공개 지표 수집, 권장 가격 전략·측정일 정산과 Slack 알림 운영 중 · 관리: 데이터 연결 담당
...
| Google Sheets | 기존 운영 화면과 `Pricing Strategies`, 성과·정산 중심 `콘텐츠`를 1분/열기 자동 동기화. Pricing Strategies 첫 칼럼과 노란색 행에 현재 권장 실험안·추천 이유를 표시. `Outreach Review`에서 가격 전략·예상 비용·최종 원문을 보고 승인. 모든 조회·수정·승인·발송은 Harper Google Workspace 로그인으로 인증 | 새 팀원은 첫 실행에서 Google 권한만 승인. 개인 GTM 키 입력·보관 없음. Google Sheets는 DB 자체 화면이 아니라 1분 안에 갱신하는 운영 복사본 |
...
대상: Harper Supabase의 `POST /rest/v1/rpc/gtm_api`. 서버와 관리형 Agent runtime은 제한된 GTM 접근 키를 사용한다. Google Sheet는 Supabase에 직접 연결하지 않고 Harper 서버의 allowlist proxy를 호출한다. 서버는 Google identity token의 서명·Apps Script OAuth client·verified email·`matchharper.com` Workspace domain을 확인한 뒤 서버 보관 키를 주입한다. 서비스 역할 키·DB 비밀번호·GTM 원본 키를 팀원이나 Sheets에 배포하지 않는다.

관리형 로컬 Agent runtime에서는 저장소의 `scripts/contents-engine/client.py`를 사용한다. 구성은 `~/.config/harper/contents-engine.json`이며 runtime 소유자만 읽을 수 있어야 한다. 클라이언트가 token을 주입하므로 요청 JSON에 키를 넣지 않는다. 이 키는 서버/자동화 운영 비밀이며 팀원 개인 키가 아니다. 팀원에게 키 발급·복사·입력을 요청하지 않고, 관리형 runtime이 없는 환경에서는 Google Sheet 또는 배포된 서버 경로를 사용한다.
...
팀원은 시트 편집 권한과 `@matchharper.com` Google Workspace 계정만 필요하다. 첫 실행에서 Google이 요구하는 권한은 현재 시트 읽기/쓰기, 외부 서비스 연결, trigger 관리, 로그인 이메일 확인이다. `Contents Engine → 연결 확인`을 실행해 한 번 승인하면 되고 연결 JSON이나 GTM 키 입력 메뉴는 없다. 1분 trigger는 설치한 Workspace 계정의 Google identity로 같은 서버 경로를 호출한다. 서버 비밀값은 셀·Apps Script 소스·UserProperties·Notion에 쓰지 않는다.

2026-09-17의 최초 검증에서는 팀원별 전용 키 경로를 사용했지만 1.12에서 폐기했다. 현재 검증 기준은 Google 권한 승인 → 서버 bridge 조회 → 전체 새로고침 → 임시 포맷 한 건 저장 → 동일 ID 재조회 → 검증 행·감사 기록 제거 → 다시 새로고침이다. Format Bank·Outreach Templates, 기존 Creator Directory 행의 안전 이관, 1분/열기 trigger와 행 단위 병합도 같은 identity 경로에서 확인한다. 검증용 업무 행은 남기지 않는다. 현재 승인된 Outreach Template은 한국 개발·커리어 크리에이터용 email rate 문의형과 Instagram DM 관심 확인형 2건이며, 실제 draft/sent/reply 수치는 해당 원장과 Outreach Log에서 읽는다. 입력 수정은 즉시 DB에 쓰지 않고 ‘미반영’으로 표시하며 선택행 저장 메뉴를 누른다. 서버 변경과 충돌하면 로컬 수정은 남기고 재확인을 요구한다. 새로고침은 미반영 수정 행만 보존하면서 같은 탭의 신규·변경 DB 행을 계속 갱신한다. 통신 결과가 불명확한 쓰기는 같은 요청 ID로 재시도한다. 숨김 ID/버전 칼럼과 조회값은 수정 경고를 표시한다.

Creator Directory는 전체 후보의 조사 정보와 간단한 연락·연결 상태를 표시하고 허용된 크리에이터 원본 칼럼만 편집한다. 흰색 칸은 편집 가능하고 회색 칸은 Supabase 동기화 값이다. Owner와 Primary Platform 칼럼은 없으며 연결 플랫폼은 `Platforms`에 모으고 `Languages`는 Outreach Status·Last Contact At 바로 오른쪽에 둔다. Connected Creators는 실제 연락·협업 이력이 있는 크리에이터의 협업·콘텐츠·비용·성과·채택 방향을 모은 읽기 화면이다. Outreach Log는 메시지 초안·발송·수신 원문과 회신 분류·게시 확인 필요·반송·수동 발송 상태를 관계 정보와 함께 시간순으로 보여준다. Format Bank는 제작 기준을, Outreach Templates는 메시지 기준을, Pricing Strategies는 고정비 또는 기본비+조회수 연동 조건을 관리한다. Outreach Review는 크리에이터·프로필 URL·수신 이메일·템플릿·가격 전략·예상 비용·제목·본문·결정을 앞쪽에 보여준다. 콘텐츠 시트는 게시물 URL과 공개 지표·측정일·예상/확정 지급액을 앞에 두고 ref는 뒤에 둔다. 결정 칸은 검토 전 노란색이며 승인 선택 또는 실제 발송 시각이 있는 행은 초록색이다. 1분 주기와 파일 열기 trigger가 DB View/RPC를 다시 읽는다. 미반영 편집은 해당 행만 보존하므로 같은 탭의 신규 DB 행과 다른 행의 서버 변경은 계속 들어온다. 성공은 A3의 초록 상태, 실패는 원인과 시각이 있는 빨간 상태로 표시한다. Gmail 회신과 측정일 지급액 확정은 Slack `_growth_creators`에 짧게 알린다.

측정일 정산 Slack은 콘텐츠 한 건당 한 메시지이며 `크리에이터명 + 클릭 가능한 콘텐츠`, `조회수·좋아요·작성자 제외 댓글`, `지급 예정액`, `80자 이내 성과 결론`만 기본 표시한다. 내부 ref·UUID·수집기 진단·긴 가격 계산식은 정상 메시지에서 제외한다. 성과 결론은 GLM 5.3 Flash가 비교 가능한 근거를 읽어 작성하고, 모델 실패나 주요 결측 시 `판단 보류`로 표시한다. 이 결론은 팀원이 채택한 Scale/Retry 결정이 아니며 실제 송금 완료도 아니다.

`gtm_access_tokens`는 접근 관리용 내부 테이블이다. 원본 키 대신 SHA-256 해시·이름·읽기/쓰기·만료/폐기를 저장한다. 서버 bridge와 관리형 Agent runtime용 키를 분리해 90일 만료로 운용하며 둘 다 팀원 개인에게 전달하지 않는다. Sheet proxy는 네 개의 필요한 RPC와 각 허용 필드만 통과시키고 Outreach Review의 actor email은 검증된 Google identity에서 서버가 넣는다.

401/접근 오류이면 팀원에게 GTM 키를 요구하지 않는다. 먼저 해당 팀원이 `@matchharper.com` 계정으로 Google 권한을 승인했는지, Apps Script OAuth client가 서버 allowlist와 일치하는지 확인한다. 이어 서버 bridge 키의 만료/폐기와 자동 동기화 trigger 실행 로그를 확인하고, 필요하면 관리자가 서버 비밀 저장소에서 키를 교체한다. 40001/버전 충돌은 최신 행과 차이를 읽고 재적용한다. 타임아웃은 같은 논리 request_id로 결과를 확인/재시도한다. 인증 오류를 피하려고 RLS를 끄거나 서비스 역할 키를 Sheet에 배포하지 않는다.
