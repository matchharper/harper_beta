...
조회수 연동 가격 전략의 측정일이 되면 측정일 이후 처음 성공한 한 번의 수집에서 `views`, `likes`, `comments_non_author`를 같은 근거와 기준 시각으로 묶는다. 비용 계산에는 이 스냅샷의 조회수만 사용한다. 이후 누적 수치가 늘어나도 이미 확정된 지급 예정액과 당시 Slack 보고를 다시 쓰지 않는다.

Slack 보고에는 크리에이터명, 클릭 가능한 콘텐츠 제목, 조회수, 좋아요 수, 작성자 제외 댓글 수를 넣는다. 숨김·미노출·수집 실패 값은 0으로 바꾸지 않고 `확인 불가`로 표시한다. 작성자 제외 댓글은 공개 댓글 수집 범위 안에서 계산된 값이며, 플랫폼 전체 댓글에서 수집기가 확인한 작성자 댓글만 뺀 경우에는 그 한계를 저장한다. 이 한계는 DB 근거에는 남기되 정상적인 Slack 메시지에 장황하게 반복하지 않는다.
...
| 날짜 | 버전 | 변경 |
| --- | --- | --- |
| 2026-09-18 | 1.3 | 가격 전략 측정일에 조회수·좋아요·작성자 제외 댓글을 같은 스냅샷으로 고정해 정산·Slack 보고에 넘기는 계약 추가 |
